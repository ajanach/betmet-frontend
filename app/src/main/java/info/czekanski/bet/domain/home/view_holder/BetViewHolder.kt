...
class BetViewHolder(
    ...
    fun bind(cell: BetCell) {
        ...
        val smallText = when {
            isAfterMatch && userBet?.score == cell.bet.match?.score -> "POGODIO"
            isAfterMatch && userBet?.score != cell.bet.match?.score -> "PROMAŠIO"
            !isAfterMatch && cell.bet.match?.state == MatchState.DURING -> "TRAJE"
            else -> ""
        }
